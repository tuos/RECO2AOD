...
void mbbarplotround3() {
   ...
   double vcollv2[nx+1];
   double vcollv3[nx+1];
   ifstream myfile ("stimingmbround3.txt");
   if (myfile.is_open()){
     for(int i=0; i<nx; i++){
       int ich=0;
       int icoll=0;
   ...
   vcollv3[np-1]=sum-sum25;


double percent[nx];
   TH1D *h0 = new TH1D("h0","",26,0,26);
   for (int i=0; i<np; i++) {
      percent[i]=vcollv3[i]*100.0/sum;
...
c1->SaveAs("timingMBAOD_1000events_round3.png");
